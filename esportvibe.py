        ...
        with sr.Microphone() as source:
            try:
                ...
                for exercicio in exercicios:
                    links.extend(buscar_videos_youtube(exercicio))
                ...
                if "Erro" in links[0]:
                    ...
                    for i, link in enumerate(links[:3]):  # Limita a 3 links
                        botao_video = ctk.CTkButton(
                            botoes_frame,
                            text=f"Vídeo {i + 1}: {exercicios[i]}",
...
texto_label = ctk.CTkLabel(root, text="Reconhecimento de voz", font=("Arial", 16), wraplength=580)
texto_label.pack(pady=20)

# Label para exibir os resultados
resultado_label = ctk.CTkLabel(root, text="", font=("Arial", 14), wraplength=580)
...
botao = ctk.CTkButton(root, text="Iniciar Reconhecimento", command=reconhecer_fala)
botao.pack(pady=10)
